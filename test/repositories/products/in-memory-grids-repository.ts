...
export class InMemoryGridsRepository implements GridsRepository {
  private items: Grid[] = []

  async create(data: Grid): Promise<void> {
    this.items.push(data)
  }

  async save(answer: Grid): Promise<void> {
  ...
  async findByName(name: string): Promise<Grid | null> {
    return this.items.find((i) => i.name.value === name) ?? null
  }

  async list(options: PaginationParams): Promise<ListingResponse<Grid>> {
